...
2. Run the Server: 
   $ cd server $ sbt compile $ sbt run 

3. Run the Client: 
   $ cd client $ sbt compile $ sbt “run n”

Note: Here the ‘n’ refers to the number of users (clients) to simulate e.g. $ sbt “run 10” Note: The maximum number of users that we could simulate is 10,000 
...
Scenario 1: Client actor “posts” some data to the server. 
...
Scenario 2: Client actor “gets” some data from the server. 
